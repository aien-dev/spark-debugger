...
fn cortex_token_path() -> std::path::PathBuf {
...
fn hive_db_path() -> std::path::PathBuf {
    let home = std::env::var("HOME")
        .or_else(|_| std::env::var("USERPROFILE"))
        .unwrap_or_else(|_| ".".to_string());
    std::path::PathBuf::from(home).join(".config/cortex/hive.db")
}
...
pub async fn report_incident(client: &Client, report: &SystemAuditReport, failure_summary: &str) {
    ...
    let tok_path = cortex_token_path();
    if let Ok(token) = std::fs::read_to_string(&tok_path) {
        let payload = json!({
            "space": "atlas-memory",
...
Report Details: {:?}", failure_summary, report)
    ...
    let h_path = hive_db_path();
    if h_path.exists() {
        if let Ok(conn) = rusqlite::Connection::open(&h_path) {
